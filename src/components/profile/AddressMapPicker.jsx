  ...
  useMapEvents({
    click: (e) => {
      if (!disabled && !isLocationLocked) {
        ...
        console.log('🗺️ [MapClickHandler] Click detectado en:', { lat, lng });
...
const AddressMapPicker = ({ 
  ...
  const [popupAddressInfo, setPopupAddressInfo] = useState(null);
  ...
  const handleLocationSelect = useCallback(async (coordinates) => {
    console.log('🗺️ [AddressMapPicker] handleLocationSelect llamado con:', coordinates);
    console.log('🗺️ [AddressMapPicker] Estados actuales:', {
      disabled,
      isProcessingRef: isProcessingRef.current,
      isLocationLocked
    });
    
    if (disabled || isProcessingRef.current || isLocationLocked) {
      console.log('🗺️ [AddressMapPicker] Selección bloqueada por:', {
        disabled,
    ...
    setError(null);
    setShowLocationPanel(true); // SIEMPRE mostrar el panel
    setIsLocationLocked(false);
    
    console.log('🗺️ [AddressMapPicker] Ubicación seleccionada:', coordinates);
    console.log('🗺️ [AddressMapPicker] showLocationPanel establecido a TRUE');
    ...
    try {
      ...
      if (geocodingResult.success) {
        ...
        console.warn('⚠️ [AddressMapPicker] Geocoding falló, usando información básica');
        // Si falla el geocoding, usar información básica
        const basicAddress = {
          fullAddress: `Ubicación en ${coordinates.lat.toFixed(6)}, ${coordinates.lng.toFixed(6)}`,
          street: '',
          city: 'El Salvador',
          state: 'El Salvador',
          country: 'El Salvador'
        };
        setAddressInfo(basicAddress);
        setPopupAddressInfo(basicAddress);
  ...
  // Debug: Log del estado del panel
  useEffect(() => {
    console.log('🗺️ [AddressMapPicker] Estado del panel:', {
      showLocationPanel,
      addressInfo: !!addressInfo,
      deliveryTimeInfo: !!deliveryTimeInfo,
      currentLocation: !!currentLocation,
      popupAddressInfo: !!popupAddressInfo
    });
  }, [showLocationPanel, addressInfo, deliveryTimeInfo, currentLocation, popupAddressInfo]);
  ...
  // Información para mostrar en el panel (priorizar addressInfo, fallback a popupAddressInfo)
  const displayAddressInfo = addressInfo || popupAddressInfo;

  console.log('🗺️ [AddressMapPicker] Renderizando con showLocationPanel:', showLocationPanel);
  ...
  return (
    <>
                      ...
                      <div className="marker-popup">
                        ...
                        {popupAddressInfo && (
                          <div>
                            ...
                            <p>{popupAddressInfo.fullAddress}</p>
                            {popupAddressInfo.street && (
                              <p><strong>Calle:</strong> {popupAddressInfo.street}</p>
                            )}
                            {popupAddressInfo.city && (
                              <p><strong>Ciudad:</strong> {popupAddressInfo.city}</p>
                            )}
                            {popupAddressInfo.state && (
                              <p><strong>Departamento:</strong> {popupAddressInfo.state}</p>
                            )}
                          </div>
                        )}
                      </div>
      ...
      {/* Panel de información de ubicación - FUERA del mapa, debajo - SIEMPRE VISIBLE CUANDO showLocationPanel ES TRUE */}
      {showLocationPanel && (
        <div className="location-info-panel-container">
          <div className="location-info-panel always-visible">
            ...
            <div className="location-info-content">
              {displayAddressInfo && (
                <div className="location-detail">
                  ...
                  <span className="detail-value">{displayAddressInfo.fullAddress}</span>
                </div>
              )}
              
              {displayAddressInfo && displayAddressInfo.street && (
                <div className="location-detail">
                  <span className="detail-label">Calle:</span>
                  <span className="detail-value">{displayAddressInfo.street}</span>
                </div>
              )}
              
              {displayAddressInfo && displayAddressInfo.city && (
                <div className="location-detail">
                  <span className="detail-label">Ciudad/Municipio:</span>
                  <span className="detail-value">{displayAddressInfo.city}</span>
                </div>
              )}
              
              {displayAddressInfo && displayAddressInfo.state && (
                <div className="location-detail">
                  <span className="detail-label">Departamento:</span>
                  <span className="detail-value">{displayAddressInfo.state}</span>
                </div>
              )}
